...
const CardPoke = ({data=[],design=1,...others}) =>{
  // console.log(data)
  let styleCommon={
    borderRadius:"10px",padding:"2px 10px",color:"#ffffff",lineHeight:"1.3",fontWeight:"600"
  }
  let styleTypes=[
    {
      typePoke:'grass',
  ...
  let colorback=styleTypes.find((st) => st.typePoke===data.types[0].type.name)?.style?.background
  // console.log(colorback)
  return(
    <div className="cardPoke" {...others}>

      <div className={design===1?'cardPoke-back':'cardPoke-back2'} style={{background:colorback}}></div>
      <div className="cardPoke-container">
        ...
        <div className="cardPoke-types">
          {data.types.map((typePoke)=>{
            return <span key={typePoke.type.name} style={styleTypes.find((st) => st.typePoke===typePoke.type.name)?.style}>{typePoke.type.name}</span>
          })}
        </div>
      </div>
